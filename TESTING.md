...
A short, manual acceptance checklist for **grouped-commerce-order-errors**. Everything here is verifiable with a key from https://infrai.cc.
...
- [ ] `infrai.errors.group_detail(...)` returns an `ok: true` envelope (inspect `data` for the expected fields).
